...
@Component({
  ...
  template: `
    <div class="cx-animate-in">
      <cx-page-header title="Approval Workflows" subtitle="Configure loan approval workflows">
...
export class ApprovalWorkflowsComponent implements OnInit {
  columns: TableColumn[] = [{key:'name',label:'Workflow Name'},{key:'product_name',label:'Product'},{key:'mode',label:'Mode'},{key:'is_active',label:'Active'},{key:'created_at',label:'Created',type:'date'}];
  rows = signal<any[]>([]);
  loading = signal(true);
  ...
  load(params?: any): void {
    ...
    this.api.get('/approval-workflows', query).subscribe({
      next: res => {
