...
@Component({
  ...
  template: `
    <div class="cx-animate-in">
      <cx-page-header title="Notification Templates" subtitle="Manage notification templates and delivery">
...
export class NotificationsComponent implements OnInit {
  columns: TableColumn[] = [{key:'name',label:'Name'},{key:'code',label:'Code'},{key:'channel',label:'Channel'},{key:'event_trigger',label:'Event'},{key:'is_active',label:'Active'},{key:'created_at',label:'Created',type:'date'}];
  rows = signal<any[]>([]);
  loading = signal(true);
  ...
  load(params?: any): void {
    ...
    this.api.get('/notification-templates', query).subscribe({
      next: res => {
