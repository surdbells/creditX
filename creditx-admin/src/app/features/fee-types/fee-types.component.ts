...
export class FeeTypesComponent implements OnInit {
  columns: TableColumn[] = [{key:'name',label:'Fee Name'},{key:'code',label:'Code'},{key:'description',label:'Description'},{key:'is_system',label:'System'},{key:'is_active',label:'Active'},{key:'created_at',label:'Created',type:'date'}];
  rows = signal<any[]>([]);
  loading = signal(true);
  ...
  load(params?: any): void {
    ...
    this.api.get('/fee-types', query).subscribe({
      next: res => {
