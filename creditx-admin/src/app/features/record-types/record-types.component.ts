  ...
  template: `
    <div class="cx-animate-in">
      <cx-page-header title="Record Types" subtitle="Manage government employee record types">
        @if (auth.hasPermission('record_types.create')) {
...
export class RecordTypesComponent implements OnInit {
  columns: TableColumn[] = [{key:'name',label:'Name'},{key:'code',label:'Code'},{key:'description',label:'Description'},{key:'is_active',label:'Active'},{key:'record_count',label:'Records'},{key:'created_at',label:'Created',type:'date'}];
  rows = signal<any[]>([]);
  loading = signal(true);
  ...
  load(params?: any): void {
    ...
    this.api.get('/record-types', query).subscribe({
      next: res => {
