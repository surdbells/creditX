...
export class PenaltyRulesComponent implements OnInit {
  columns: TableColumn[] = [{key:'name',label:'Rule Name'},{key:'product_name',label:'Product'},{key:'calculation_type',label:'Type'},{key:'value',label:'Value'},{key:'grace_period_days',label:'Grace Days'},{key:'max_amount',label:'Max Amount',type:'currency'},{key:'is_active',label:'Active'}];
  rows = signal<any[]>([]);
  loading = signal(true);
  ...
  load(params?: any): void {
    ...
    this.api.get('/penalty-rules', query).subscribe({
      next: res => {
