...
export class AccountingComponent implements OnInit {
  columns: TableColumn[] = [{key:'account_name',label:'Account Name'},{key:'account_number',label:'Acc. Number'},{key:'account_code',label:'Code'},{key:'account_type',label:'Type'},{key:'ledger_type',label:'Ledger Type'},{key:'is_active',label:'Active'},{key:'created_at',label:'Created',type:'date'}];
  rows = signal<any[]>([]);
  loading = signal(true);
  ...
  load(params?: any): void {
    ...
    this.api.get('/gl-accounts', query).subscribe({
      next: res => {
