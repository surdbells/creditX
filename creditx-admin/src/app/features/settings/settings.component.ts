...
@Component({
  ...
  template: `
    <div class="cx-animate-in">
      <cx-page-header title="System Settings" subtitle="Configure system behavior">
...
export class SettingsComponent implements OnInit {
  columns: TableColumn[] = [{key:'key',label:'Key'},{key:'value',label:'Value'},{key:'type',label:'Type'},{key:'category',label:'Category'},{key:'description',label:'Description'}];
  rows = signal<any[]>([]);
  loading = signal(true);
  ...
  load(params?: any): void {
    ...
    this.api.get('/settings', query).subscribe({
      next: res => {
