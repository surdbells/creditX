...
@Component({
      ...
      <div class="cx-aq-modal"
        ...
        <div class="cx-aq-modal-body">
          @if (mode() === 'quick-approve') {
            ...
            <!-- Bank statement (captured by the agent) -->
            @if (l.bank_statement_mode || l.account_statement_id || l.account_statement_password) {
              <section class="cx-aq-section">
                <h3 class="cx-aq-section-title">Bank Statement</h3>
                <div class="cx-aq-grid">
                  <div class="cx-aq-field">
                    <span class="cx-aq-field-label">Statement Mode</span>
                    <span class="cx-aq-field-value">{{ stmtMode(l.bank_statement_mode) }}</span>
                  </div>
                  <div class="cx-aq-field">
                    <span class="cx-aq-field-label">Statement ID</span>
                    <span class="cx-aq-field-value tabular-nums">{{ l.account_statement_id || '—' }}</span>
                  </div>
                  <div class="cx-aq-field">
                    <span class="cx-aq-field-label">Statement Password</span>
                    <span class="cx-aq-field-value cx-aq-secret">
                      @if (l.account_statement_password) {
                        <span class="tabular-nums">{{ showStmtPassword() ? l.account_statement_password : '••••••••' }}</span>
                        <button type="button" class="cx-aq-secret-btn" (click)="showStmtPassword.set(!showStmtPassword())"
                                [title]="showStmtPassword() ? 'Hide' : 'Show'">
                          <lucide-icon [name]="showStmtPassword() ? 'eye-off' : 'eye'" [size]="13"></lucide-icon>
                        </button>
                        <button type="button" class="cx-aq-secret-btn" (click)="copyText(l.account_statement_password)" title="Copy">
  ...
  styles: [`
    ...
    .cx-aq-secret-btn {
      ...
      background: none; border: 1px solid var(--cx-border); border-radius: 6px;
...
export class ApprovalQueueComponent implements OnInit {
  ...
  // ── Bank statement (shown in the detail view for reviewers) ──
  showStmtPassword = signal(false);

  /** Human-readable bank-statement mode label. */
  stmtMode(m: string | null | undefined): string {
    switch (m) {
      case 'generated_by_company': return 'Generated by company';
      case 'not_generated_by_company': return 'Not generated by company';
      default: return m || '—';
    }
  }

  /** Copy a value (e.g. the statement password) to the clipboard. */
  ...
  closeModal() {
    ...
    this.recalcConfirmed.set(false);
    this.recalcBusy.set(false);
    this.showStmtPassword.set(false);
  }
