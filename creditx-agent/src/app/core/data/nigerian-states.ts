 ...
 *
 * Used by the agent loan-capture wizard to populate cascading dropdowns
 * on Step 3 (Contact section). States are the 36 federal states plus
 * the Federal Capital Territory (FCT), which is administratively similar
 * but technically not a "state" — we include it in the same list because
 * citizens born there use it as their state of origin on forms.
 *
 * Why hardcoded instead of a backend endpoint:
 *   - Data is static (states and LGAs don't change meaningfully)
 *   - Zero network round-trip on every form render
 *   - Works offline — important for mobile agents in areas with
 *     spotty coverage
 *   - No backend dependency to maintain a states table
 *
 * Total size: ~15 KB as plain TS. Gzipped in the bundle it's ~4 KB.
 * Negligible compared to the app bundle.
 *
 * Source: Nigerian National Population Commission's LGA list, verified
 * against INEC's 2023 voter-ward mapping. 774 LGAs total.
 */

export interface StateData {
  /** Full state name as used on Nigerian government forms. */
  name: string;
  /** Local government areas within the state. Alphabetized. */
  lgas: string[];
}
...
export const NIGERIAN_STATES: StateData[] = [
  ...
  {
    name: 'Ogun',
    lgas: ['Abeokuta North', 'Abeokuta South', 'Ado-Odo/Ota', 'Egbado North', 'Egbado South', 'Ewekoro', 'Ifo', 'Ijebu East', 'Ijebu North', 'Ijebu North East', 'Ijebu Ode', 'Ikenne', 'Imeko Afon', 'Ipokia', 'Obafemi Owode', 'Odogbolu', 'Odeda', 'Ogun Waterside', 'Remo North', 'Shagamu'],
  },
  {
    name: 'Ondo',
...
/** Quick state-name lookup. O(1) LGA retrieval. */
const STATES_BY_NAME: Map<string, StateData> = new Map(NIGERIAN_STATES.map(s => [s.name, s]));

/**
 * Get LGAs for a given state name. Returns empty array if state name
 * doesn't match one of our 37 entries (safest fallback — the form
 * shows 'Select state first' instead of a stale LGA list).
 *
 * Case-sensitive by design. State names match the exact display form
 * ('Akwa Ibom', 'FCT', 'Cross River') to avoid bugs from a user selecting
 * 'Lagos' while prefill wrote 'lagos'.
 */
export function getLgasForState(stateName: string | null | undefined): string[] {
  if (!stateName) return [];
  return STATES_BY_NAME.get(stateName)?.lgas ?? [];
}
