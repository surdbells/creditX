...
// ─── Loan ───
/**
 * What an investment has earned — computed server-side so the portal never
 * re-derives money. Fixed-term investments carry projected_* (they run to a
 * known maturity); open-ended carry indicative_annual_* instead.
 */
export interface InvestmentPerformance {
  investment_number: string;
  type: 'fixed_term' | 'open_ended';
  status: string;
  principal: string;
  balance: string;
  ...
  interest_earned_to_date: string;
  interest_paid_to_date: string;
  wht_withheld_to_date: string;
  placement_date: string;
  maturity_date: string | null;
  days_invested: number;
  days_to_maturity: number | null;
  projected_gross_interest: string | null;
  projected_net_interest: string | null;
  projected_maturity_value: string | null;
  indicative_annual_gross: string | null;
  indicative_annual_net: string | null;
}
...
export interface InvestmentTransaction {
  ...
  type: string;
  amount: string;
  gross_interest: string | null;
  wht_amount: string | null;
  net_interest: string | null;
