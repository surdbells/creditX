...
/**
 * POST /api/investments/{id}/withdraw — take money out of an open-ended
 * investment. Interest is accrued to the withdrawal date first, so the
 * investor earns to the day. Gated by investments.transact.
 *
 * Body: amount, settlement_gl_id, value_date, close_if_zero.
 */
final class WithdrawInvestmentAction
{
    use ApiResponse;
    ...
    public function __invoke(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        ...
        $closeIfZero = filter_var($data['close_if_zero'] ?? false, FILTER_VALIDATE_BOOLEAN);
        ...
        try {
            $txn = $this->service->withdraw($inv, $amount, $date, $settlement, $userId, $closeIfZero);
        ...
        return $this->created($txn->toArray() + ['investment' => $inv->toArray()], 'Withdrawal posted successfully');
    }
}
