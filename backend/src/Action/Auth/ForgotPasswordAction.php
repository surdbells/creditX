...
final class ForgotPasswordAction
{
    ...
    public function __invoke(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
    {
        ...
        $resetUrl = ($_ENV['FRONTEND_URL'] ?? 'https://admin.dostsuite.com') . '/auth/reset-password?token=' . $token;
        $apiKey = $_ENV['ZEPTOMAIL_API_KEY'] ?? '';
        if ($apiKey !== '') {
            $payload = [
                'from' => ['address' => $_ENV['ZEPTOMAIL_FROM_EMAIL'] ?? 'noreply@dostsuite.com', 'name' => 'CreditX'],
